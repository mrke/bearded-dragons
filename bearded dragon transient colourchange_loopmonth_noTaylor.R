...
for(i in 0:(interval*(days+1)-1)){
  ...
  thresh<-max(vtmin,Tair_shd+airoff) # threshold body temperature at which time is required (deg C)
  ...
  if(i==0){ # starting at night in the shade, normal to the sun ready for it to come up
    ...
    if(Zen==90){ # sun is down, cooling in shade
      ...
      if(dayresults[i,2] < baskthresh){ # too cold to emerge and bask
        ...
        if(dayresults[i,2]<max(vtmin,Tair_shd+airoff) & out==0 & dayresults[i,2]<vtmax ){ # hasn't yet emerged, but sun is up so start to bask (really, there would be a threshold before they come out to bask)
          Tc_init<-dayresults[i,2]
          Qsol<-Qsol_sun
          Tair<-Tair_sun
          Trad<-Trad_sun
          ...
          if(dayresults[i,2]<vtmax & dayresults[i,2] >= max(vtmin,Tair_shd+airoff)){
            if(dayresults[i,5]==3 & dayresults[i,2]>max(vtmin,Tair_shd+airoff) & dayresults[i,8]>max(vtmin,Tair_shd+airoff)){ # stay cooling
              Tc_init<-dayresults[i,2]
              Qsol<-Qsol_shd
              Tair<-Tair_shd
              Trad<-Trad_shd
            ...
            if(dayresults[i,2]<max(vtmin,Tair_shd+airoff) & dayresults[i,2]<vtmax & dayresults[i,8]>=max(vtmin,Tair_shd+airoff)){ # animal was forced inactive, perhaps in the shift form normal to average silhouette area, so check that the hour before Te value was >= vtmin and if so try basking/foraging again
              Tc_init<-dayresults[i,2]
              Qsol<-Qsol_sun
              Tair<-Tair_sun
              Trad<-Trad_sun
              ...
              if(Tbs$Tc<vtmax){ # this works so bask
                ...
                if(Tbs2$Tc<vtmax & Tbs2$Tc>max(vtmin,Tair_shd+airoff) & dayresults[i,8]<vtmax){ # this works so forage
                  dayresults[i+1,1]<-i*step/60
